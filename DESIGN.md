...
- **Light-only, deliberately:** `theme.css` sets `color-scheme: light` and explicitly overrides `--pico-form-element-background-color`/`-border-color`/`-color`/`-placeholder-color`. Without this, every text input on every page (including the login screen) silently inherited Pico's separate `@media (prefers-color-scheme: dark)` form-element palette on any OS/browser set to prefer dark colors, with no `data-theme` attribute ever set anywhere in the app to opt back out — every other token here correctly stayed on the intended light palette, but this one variable family was missed in the original color-cascade fix and only caught by a later audit.
