...
- **Auth page:** breaks from the app-shell pattern — a single narrow (`26rem`) card, vertically centered via a flex body (`body.auth-page`) rather than sharing the persistent nav. **Six pages now share this shell.** Four are served by `redirect`: the password prompt (`prompt.html`) plus its three error pages (`error-404.html`/`error-500.html`/`error-503.html`, `docs/plans/redirect-error-pages.md`). The other two are served by `gui-pages`: its own 404/500 (`gui-pages/errorpages.py`, `docs/plans/gui-pages-error-pages.md`) — no new class, no new token, `.auth-page`/`.auth-shell` reused verbatim by both components.
...
- **Minimum tap target:** `min-height: 44px` on every button/submit/reset/`[role=button]`, sitewide — an audit measured every button at 40px, under the commonly-cited 44px touch-target guideline, on a system that has real mobile-specific layout (column collapse, sticky action column, stacked row actions). Implemented as a height floor rather than restoring `--pico-form-element-spacing-vertical` toward Pico's default, since that variable also drives every text input's tap target and reducing the tightening would undo this app's deliberate admin-density type scale. **The floor originally missed plain `<a>` nav links** — a later audit measured the persistent nav's own `.brand-link` and "Manage users" link at 38.4px, since the button-family selector never covered anchors; extended via a separate `#app-header nav a { display: inline-flex; align-items: center; min-height: 44px; }` rule (see Navigation).
  - **The floor's real shipped shape, in `gui/theme.css`, is wider than the paragraph above once recorded** — an Impeccable critique (2026-08-08) found four covered selector families and everything else sitting under the floor, closed in the same pass rather than piecemeal: the button family additionally gets `min-width: 44px` ("the guideline is a target, not a height" — `.edit-btn` measured 39.8px *wide*, tall enough but not wide enough); every `input:not([type=checkbox]):not([type=radio])`, `select` and `textarea` gets `min-height: 44px`; checkboxes and radios are **deliberately excluded** from that rule (a 44px-tall *checkbox* would be a 44px-tall box, not the guideline's intent) and instead get the floor moved to their wrapping `<label>` via `label:has(> input[type=checkbox]) { min-height: 44px; ... }`, or to `.checkbox-hit` for the two label-less table checkboxes — **hit area 44×44, visual box still 20×20**, the "expand the target, not the ink" pattern this feature's own chip decision cites directly; `details > summary` gets `padding-block: 0.875rem` (padding rather than height, since Pico's floated disclosure chevron doesn't survive becoming a flex child); `#links-table th.sortable`/`#users-table th.sortable` get `height: 44px` (a real control — `tabindex="0"`, Enter/Space cycle the sort); and `.operator-link`/`.slug-chip-link` (the in-body nav alternative and the slug-as-link idiom) both get the same `inline-flex; align-items: center; min-height: 44px` treatment as the nav anchors. `TASKS.md`'s completed audit line records the result: *"172 interactive elements across all five pages, zero under the floor."*
...
- **Input chip (`.tag-chip-editable`)** — a different component in a different context from the inert in-row tag chip above, even though it shares the same ink. Introduced by `docs/plans/tag-ux-chips-and-filtering.md` for the chip-input control (create form, bulk-create panel, bulk bar, edit rows, the tag filter). It is a real `<button type="button">`, and therefore takes the sitewide 44×44 tap-target floor **with no exemption and no new CSS to reach it** — the floor already covers the `button` family, so the chip inherits it for free. Its ink is the shared `.tag-chip` treatment, byte-identical to the inert chip's: `0.75rem` caption, sans-serif, `--ss-slate-500` (light) / dark counterpart. Its `border-radius` is `var(--pico-border-radius)` (measured `4px`), **never the pill** — the Pill-Is-For-Links Rule holds for the input chip exactly as it does for the inert one. **Why the same 44px rule reads differently here than in a table row, in one sentence:** ten interactive chips in a table nobody opened would be up to 300 extra focus stops across thirty rows nobody asked to tab through, while ten chips in a form the user deliberately opened (which already has 8 other controls) are ten stops in a place the user is already navigating — same rule, same element, opposite verdict, because the context changes what "adding a focus stop" costs, not because the floor bends. **Measured (2026-08-24), against the chip's real rendered background — the card, since the chip sets `background: none`:** ink 6.36:1 (light) / 7.58:1 (dark), both comfortably AA; hairline border 1.36:1 (light) / 1.62:1 (dark), both under WCAG 1.4.11's 3:1 non-text minimum. **No border fix was applied.** Unlike `.domain-select`/`#logout-btn` — where the border was the *only* thing marking a filled control's boundary against the nav — the input chip's own label text and `×` glyph already carry the "this is an interactive control" affordance in both themes, so the border is not the sole identifying boundary the 3:1 rule protects. If a future variant of this chip ever drops the visible label or glyph, re-run this measurement before assuming the same verdict holds.
- **"— deleted account" suffix**, on an owner name in the dashboard's owner filter and in the row editor's owner select. Not a badge and not a colour — a plain text suffix on the option label, because a `<option>` cannot contain markup and inventing a parallel treatment for the two places owners are listed would fragment the pattern. In the table's Owner cell the same fact renders as the existing `status-disabled` badge (no new token, no new contrast measurement). An orphaned owner's option in the row editor is additionally `selected disabled`: it tells the truth rather than silently pre-selecting the first real user, and it keeps an unrelated save from reading as an owner change and firing a spurious reassign confirmation.
- **The filter affordance is a chip input plus a combining-rule `<select>`, not the single-select this section used to describe.** The in-row table chips referenced above stay inert `<span>`s — that half of the sentence stays true and is not reopened. What changed (`docs/plans/tag-ux-chips-and-filtering.md`): the dashboard's `#tag-filter` single-`<select>` was replaced by a multi-tag chip input (`#tag-filter-chips`, the same component described below) plus a two-option `<select id="tag-rule">` stating how the active chips combine — "All of these tags" (the default) or "Any of these tags". A `<select>` is still the right affordance for the *combining rule* specifically, for the reasons `#owner-filter` already established in this row: narrower than a segmented button pair, one focus stop, and its rendered value already states the answer with no `aria-pressed` bookkeeping needed. `#tag-rule` is hidden below two active chips (see Components, below) — with one chip, all-of and any-of are provably identical, so a visible control that cannot change anything is clutter.

- **Why all-of by default, not any-of.** Three reasons, the third decisive: all-of is monotone-narrowing (what "filter" means; any-of *grows* the set as chips are added, which is the surprise this feature exists to avoid); it keeps one consistent AND semantics across every filter on the row (`#links-filter`, `#owner-filter`, and the tag chips among themselves would otherwise carry two different combining rules with nothing on screen distinguishing them); and — decisively — **the filtered set feeds bulk Delete/Reassign**, and any-of's over-matching plus select-all is how an operator deletes or reassigns links they never meant to touch. Any-of is still offered, one `<select>` away, because "everything in either campaign" is a real question; it is just never the default a viewer lands on unintentionally.
- **`#filter-summary`** states the whole active filter in words plus the count (e.g. "Showing 7 of 42 links · tags: all of #q4, #sale · owner: alice"), immediately below the filter row. It reuses `.form-note` — **no new token, no new colour** — and is hidden entirely when no filter is active. **Deliberately not a live region:** it rebuilds on every keystroke of `#links-filter`, and a keystroke-rate `role="status"` would be noise rather than help; `#tag-rule`'s own `<select>` already announces its value change natively, which is enough. The empty state additionally names the active tags and the combining rule when ≥2 chips return nothing — the moment all-of is most confusing is exactly the moment it returns zero rows.
...
- **Copy rules for `redirect/error-404.html`/`error-500.html`/`error-503.html`** (`docs/plans/redirect-error-pages.md`): no diagnostics (no slug, status code, error id, or host message — the templates take no data at all); no "we've been notified" (nobody is, on the paths that reach these pages); no links or navigation (a public visitor has no account, so a link to the console only points the curious at it); and **nothing in `danger-red`** — the h1 uses the default heading treatment (Deep Navy). `theme.css`'s own comment above `.form-note` states the rule these pages follow: red is reserved for "something just failed," and reusing it for a standing advisory makes a real failure look identical to permanent furniture, which is exactly what a whole page whose only content is an error message is.
- **The 404's copy additionally may never distinguish *why* a slug is unavailable** — absent, disabled, and outside its `[start_at, end_at)` window must render byte-identical text (CLAUDE.md, "Security tradeoffs"). Never add wording naming expiry, scheduling, or disablement to this page; `gui-pages/tests/test_no_inline_code.py` pins a forbidden-word list against it for exactly this reason.

### `gui-pages` error pages
- **Copy rules for `gui-pages/errorpages.py`'s two pages** (`docs/plans/gui-pages-error-pages.md`): same "no diagnostics" and "nothing in `danger-red`" rules as `redirect`'s error pages above (no requested path, status code, or host message; the `h1` uses the default Deep Navy heading treatment). Two rules differ, deliberately. First, **no "we've been notified"** — `gui-pages` has no logging at all (CLAUDE.md, "Toggleable structured logging"), so saying so would be a lie; the 500's copy points the visitor at the operator instead. Second, and the more consequential difference: **`redirect`'s "may never distinguish why" rule does NOT apply to this 404.** `gui-pages`'s `ROUTES` is a fixed, public allowlist already in the repo — there is no link-existence secret to protect the way there is at `/r/{slug}` — so this 404 names the single most likely cause (a short link missing its `/r/`) in its detail line. **Do not back-port this page's specificity onto `/r/`'s 404, and do not back-port `/r/`'s indistinguishability onto this page** — the two pages answer different questions under different constraints, and collapsing them either direction is wrong.
...
- **In-body operator links are the sanctioned alternative to a nav item**, now used twice: `admin/users.html` carries `Backup and restore · Destination URL policy` under its "Users" heading. This is what "the nav is at its item budget" means in practice — an admin surface that would otherwise want a nav entry gets a plain anchor on the page an operator is already on. **Two is still legible; a third should trigger a rethink** rather than a third middot, since the "Users" heading is already only loosely related to what the links point at.
- **A page's breadcrumb may be shorter than its own `<h1>`, and should be when the nav is tight.** `admin/url-policy.html` renders `pageLabel: "URL policy"` next to an `<h1>Destination URL policy</h1>`: the full 22-character label overflowed `#app-header nav` at 390px (`scrollWidth` 374 vs `clientWidth` 351, both themes) while `backup.html` measured a clean 351/351 at the same width, so label length was the entire difference. Shortening is safe **only where the page has a real `<h1>`** — the rule above about keeping the breadcrumb visible at narrow widths exists for `links/detail.html`, which has no heading at all and would lose its only page identity. Measure a new page's label at 390px before assuming it fits.
