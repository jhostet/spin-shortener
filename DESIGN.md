...
- **Auth page:** breaks from the app-shell pattern — a single narrow (`26rem`) card, vertically centered via a flex body (`body.auth-page`) rather than sharing the persistent nav. Four pages now share this shell, all served by `redirect` rather than `gui-pages`/`gui`: the password prompt (`prompt.html`) plus the three error pages (`error-404.html`/`error-500.html`/`error-503.html`, `docs/plans/redirect-error-pages.md`) — no new class, no new token, `.auth-page`/`.auth-shell` reused verbatim.
