...
- `route = "/api/..."` → **`api`** component (`api/`, Python) — intended to hold the shortener's data/API logic. Built with `componentize-py -w spin:up/http-trigger@4.0.0 componentize app -o app.wasm`, compiling `api/app.py` (module name `app`) to `api/app.wasm`. Uses `spin_sdk.http.Handler`.
...
Python component dependencies (`componentize-py`, `spin-sdk`) are pinned in `api/requirements.txt`:
