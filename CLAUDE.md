...
**Akamai Functions itself publishes no caching documentation at all** (`docs/caching` 404s) — whether a `*.fwf.app` app inherits Property Manager's "302 not cached by default" behaviour is **UNCONFIRMED** until measured against an actual deployment (repeatedly requesting a deployed `/r/{slug}` and confirming the click count increments exactly once per request, with no `X-Cache`/`Age` header suggesting an edge hit). Update this paragraph with the result once that check has run.
