...
- An admin-run store consistency check that reports where the data has drifted out of step with itself — links missing from an index, index entries with no link, links owned by a deleted account. The check itself only reads; where a finding has exactly one safe, derivable fix, a one-click Repair action is offered alongside it. Four kinds of finding are never auto-repaired because fixing them needs a judgement call — those are named with the ordinary reassign/delete tools or the KV explorer as the way to resolve them instead
