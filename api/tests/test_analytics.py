...
async def test_analytics_not_found():
    ...
    resp = await analytics.handle_analytics(links_store, analytics_store, _principal(), "doesnotexist", fake_get_many)
...
async def test_analytics_forbidden_for_non_owner():
    ...
    resp = await analytics.handle_analytics(links_store, analytics_store, _principal(username="bob"), slug, fake_get_many)
...
async def test_analytics_admin_can_view_others_links():
    ...
    analytics_store = FakeStore()
    slug = await _make_link(links_store, owner="alice")
    resp = await analytics.handle_analytics(links_store, analytics_store, _principal(username="admin", role="admin"), slug, fake_get_many)
...
async def test_analytics_view_all_permission_can_view_others_links():
    ...
    resp = await analytics.handle_analytics(links_store, analytics_store, viewer, slug, fake_get_many)
    assert resp.status == 200
...
async def test_analytics_edit_all_permission_can_view_others_links():
    ...
    resp = await analytics.handle_analytics(links_store, analytics_store, editor, slug, fake_get_many)
    assert resp.status == 200
...
async def test_analytics_no_clicks_yet_returns_zeros():
    ...
    resp = await analytics.handle_analytics(links_store, analytics_store, _principal(), slug, fake_get_many)
    assert resp.status == 200
...
async def test_analytics_response_keys_are_exactly_total_and_days():
    # Pins the recent-events removal (docs/plans/drop-events-write.md):
    # recent_events must never creep back into the 200 body, not even as an
    # empty list. A leftover `events:` key from before the cutover must not
    # resurface it either.
    links_store = FakeStore()
    analytics_store = FakeStore()
    slug = await _make_link(links_store)
    await analytics_store.set(f"count:{slug}:0", json.dumps({"total": 1, "days": {"2026-01-01": 1}}).encode("utf-8"))
    await analytics_store.set(f"events:{slug}:0", b"1000|https://ref.example/|desktop")

    resp = await analytics.handle_analytics(links_store, analytics_store, _principal(), slug, fake_get_many)
    assert resp.status == 200
    body = json.loads(resp.body)
    assert set(body.keys()) == {"total", "days"}
...
async def test_analytics_reports_count():
    links_store = FakeStore()
    analytics_store = FakeStore()
    slug = await _make_link(links_store)
