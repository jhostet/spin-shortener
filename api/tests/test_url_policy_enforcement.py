...
THREE authoring paths — links.handle_create, links.handle_update (only when
target_url changes) and bulk.handle_bulk_create — not just some of them.
"A policy enforced in two of three places is not enforced."
...
of links.handle_create, links.handle_update and bulk.validate_bulk_rows in
turn, one at a time, re-ran `uv run pytest tests/test_url_policy_enforcement.py`,
and restored the original source. Each removal made this module fail:
  ...
  - bulk.validate_bulk_rows' policy check removed -> test_bulk_create_rejects_
    ...
    created).
Confirms this module actually exercises all three enforcement points rather
